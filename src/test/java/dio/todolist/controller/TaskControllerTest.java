...
class TaskControllerTest {
    ...
    void listByStatusCase2() throws Exception {
        TaskResponse expectedResponse = new TaskResponse(
                DEFAULT_ID, DEFAULT_TITLE, DEFAULT_DESCRIPTION, LocalDateTime.now(), TaskStatus.CONCLUIDA);
    ...
    void updateCase1() throws Exception {
        ...
        TaskResponse expectedResponse = new TaskResponse(
                DEFAULT_ID, "Nova tarefa", "Nova descrição", LocalDateTime.now(), TaskStatus.CONCLUIDA);
    ...
    void updateCase2() throws Exception {
        TaskUpdate updateRequest = new TaskUpdate(
                Optional.of("Título atualizado"),
                Optional.empty(),
                Optional.empty()
        );
        TaskResponse expectedResponse = new TaskResponse(
                DEFAULT_ID, "Título atualizado", DEFAULT_DESCRIPTION, LocalDateTime.now(), TaskStatus.PENDENTE);
        String taskJson = objectMapper.writeValueAsString(updateRequest);
    ...
    void deleteCase2() throws Exception {
        doThrow(new AccessDeniedException("Acesso negado")).when(taskService).delete(DEFAULT_ID);
